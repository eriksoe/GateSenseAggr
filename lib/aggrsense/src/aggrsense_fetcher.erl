...
handle_info(_Info, State) ->
    ...
    error_logger:info_msg("~s: fetching completed.\n", [?MODULE]),
    {noreply, State}.
    ...
    DisplayResult =
        case parse_and_validate_json(Json) of
            ...
            {ok, Validated} ->
                ...
                {Location, Measurements} = Extracted,
                ok = aggrsense_storage:add(FeedID, Location, Measurements)
        end,
...
parse_and_validate_json(JsonStr) ->
    Spec = {object, [{<<"updated">>, string},
                     ...
                     {<<"location">>, {object,
                                       ...
                                       ?ignore_rest]}},
                     ?ignore_rest]},
    aggrsense_util:parse_and_validate_json(JsonStr, Spec).
...
%?with_trace(begin
    ...
    {_Location={_,_,_,_}, Measurements} = extract_data(Validated),
